function [a,b,x,y,M1] = genbeambending(R,psi,M,L,E,I,numpts)
...
fun = @genSolve;
var0 = [0.25,0.5,0.5];
var = fsolve(fun, var0, [], psi (1), E, I, R(1), M(1), alpha(1));
...
function F = genSolve(vari, psi, E, I, R, M, alpha)
%var = [k, phi1, phi2];
k = vari(1);
phi1 = vari(2);
phi2 = vari(3);
F(1) = k*sin(phi1)-cosd(psi/2);
F(2) = M - 2*k*sqrt(E*I*R)*cos(phi2);
if isreal(phi1)
else phi1 = pi/2;
end
if isreal(phi2)
